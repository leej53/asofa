-- =====================================================================
-- File: mimic_baseline_characteristics_sepsis.sql
-- BigQuery Standard SQL
-- Project:   <PROJECT>
-- Dataset:   <DATASET>
-- Table out: <PROJECT>.<DATASET>.mimic_baseline_characteristics_sepsis
--
-- Purpose
--   Build baseline characteristics for the Sepsis-3 cohort in MIMIC-IV v3.1,
--   combining demographics (age, gender), aSOFA/SOFA totals, LOS, and
--   Charlson components at the HADM level.
--
-- Cohort & Alignment
--   * Cohort is already restricted to adult FIRST ICU stays with Sepsis-3,
--     defined as sepsis3=TRUE AND (septic_on_admission OR sepsis_within_24h),
--     via the finalized totals table <PROJECT>.<DATASET>.mimic_asofa_total_sepsis. [ref]
--   * Sepsis-3 table (<PROJECT>.<DATASET>.sepsis3) computes
--     septic_on_admission / sepsis_within_24h flags consistently with eICU. [ref]
--   * Suspicion-of-infection timing follows the finalized abx–culture rule. [ref]
--
-- Charlson note
--   * Charlson comorbidity data come from
--     physionet-data.mimiciv_3_1_derived.charlson at (subject_id, hadm_id).
--   * The reported `charlson_comorbidity_index` here is the **age-adjusted CCI**.
--   * For mutually exclusive pairs we apply precedence:
--       severe_liver_disease > mild_liver_disease
--       diabetes_with_cc     > diabetes_without_cc
--       metastatic_solid_tumor > malignant_cancer
--
-- Inputs
--   <PROJECT>.<DATASET>.mimic_asofa_total_sepsis
...
CREATE OR REPLACE TABLE `<PROJECT>.<DATASET>`.mimic_baseline_characteristics_sepsis AS
WITH base AS (
  -- Sepsis-3 cohort totals (adult + first ICU stay + septic_on_admission/within_24h)
  SELECT
    subject_id,
    stay_id,
    length_of_stay,
    asofa_total_score,
    sofa_total_score
  FROM `<PROJECT>.<DATASET>`.mimic_asofa_total_sepsis
),

-- Demographics: encode male as 1, else 0; age from anchor_age
pats AS (
  SELECT
    subject_id,
...
-- Charlson raw (age-adjusted index + component flags) at HADM level
charlson_raw AS (
  SELECT
    c.subject_id,
    c.hadm_id,
    ...
    -- Age-adjusted Charlson Index from the derived view
