-- =====================================================================
-- File: eicu_baseline_characteristics_sepsis.sql
-- Dialect: BigQuery Standard SQL
-- Project:   <PROJECT>
-- Dataset:   <DATASET>
-- Table out: <PROJECT>.<DATASET>.eicu_baseline_characteristics_sepsis
--
-- Purpose
--   Build a Sepsis-3 cohort baseline table for eICU (adults, FIRST ICU unit
--   visit) combining demographics (age, gender), total aSOFA/SOFA, LOS/
--   30-day mortality, and Charlson comorbidity components derived from
--   `pasthistory`.
--
-- Cohort & Alignment
--   * Cohort rows come from <PROJECT>.<DATASET>.eicu_asofa_total_sepsis:
--       adult (>=18y), first ICU unit visit, and septic on arrival OR within 24h.
--     This mirrors the finalized Sepsis-3 build and naming used in public code. [ref]
--   * Header tone, column naming, and output schema follow the finalized
--     eICU baseline table format. [ref]
--   * Charlson components are obtained from pasthistory via the curated mapping
--     table <PROJECT>.<DATASET>.eicu_charlson_map. [ref]
--
-- Charlson note
...
--                          Moderate–severe liver=3; Metastatic solid tumor=6; AIDS=6.
--       Precedence (mutually exclusive): severe_liver > mild_liver;
--                                        diabetes_with_cc > diabetes_without_cc;
--                                        metastatic > malignant.
--
-- Inputs
--   <PROJECT>.<DATASET>.eicu_asofa_total_sepsis   -- sepsis cohort & totals
...
--
-- Notes
--   * `gender` is passed through from eicu_asofa_total_sepsis as-is to keep
--     consistency with your finalized totals table (numeric 1/0 or string per build).
--   * This script is GitHub-ready and aligned with other public eICU scripts.
-- =====================================================================

CREATE OR REPLACE TABLE `<PROJECT>.<DATASET>`.eicu_baseline_characteristics_sepsis AS
WITH
-- ---------------------------------------------------------------------
-- 0) Base cohort and totals (already sepsis-restricted)
-- ---------------------------------------------------------------------
base AS (
  SELECT
    patientunitstayid,
    ...
    asofa_total_score,
    sofa_total_score
  FROM `<PROJECT>.<DATASET>`.eicu_asofa_total_sepsis
),
...
-- ---------------------------------------------------------------------
-- 2) Join curated mapping; keep only mapped categories
-- ---------------------------------------------------------------------
ph_mapped AS (
  SELECT DISTINCT
...
--    "renal insufficien*" anywhere in combined strings → renal_insuff_flag=1
-- ---------------------------------------------------------------------
renal_insuff AS (
  SELECT
    ph.patientunitstayid,
...
-- ---------------------------------------------------------------------
-- 5) Apply precedence rules and build weighted components
-- ---------------------------------------------------------------------
charlson_weighted AS (
  SELECT
    b.patientunitstayid,
    -- precedence: severe > mild liver
    CASE WHEN f.has_liver_severe = 1 THEN 0 ELSE f.has_liver_mild END AS mild_liver_disease,      -- 1 pt
    CASE WHEN f.has_liver_severe = 1 THEN 1 ELSE 0 END           AS severe_liver_disease_flag,    -- 3 pts
    -- precedence: metastatic > malignant cancer
    CASE WHEN f.has_met = 1 THEN 0 ELSE f.has_malig END          AS malignant_cancer_flag,        -- 2 pts
    CASE WHEN f.has_met = 1 THEN 1 ELSE 0 END                    AS metastatic_solid_tumor_flag,  -- 6 pts
    ...
    f.has_para  AS paraplegia,     -- 2 pts
    f.has_renal AS renal_disease,  -- 2 pts
    f.has_aids  AS aids            -- 6 pts
  FROM base b
...
-- ---------------------------------------------------------------------
-- 7) Final baseline (Sepsis-3 cohort)
-- ---------------------------------------------------------------------
SELECT
  b.patientunitstayid,
  b.age,
  b.gender,  -- passed through from totals table
  b.length_of_stay,
  b.mortality_30day,
  b.asofa_total_score,
  b.sofa_total_score,
